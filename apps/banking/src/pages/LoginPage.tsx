// apps/banking/src/pages/LoginPage.tsx — sign in, with the demo credentials on screen.
//
...
export default function LoginPage() {
  ...
  const submit = () => {
    ...
    window.setTimeout(() => {
      ...
      refresh();
      toast.success('Signed in', 'Welcome back to FinSecure Bank.');
      navigate('/dashboard');
    }, 400);
  };
  ...
  return (
    <div className="grid min-h-dvh lg:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)]">
      ...
      <aside className="from-shell-900 to-shell-950 relative flex flex-col justify-between overflow-hidden bg-gradient-to-b px-6 py-10 sm:px-10 lg:px-14 lg:py-14">
        ...
        <div
          aria-hidden
          className="bg-brand-500/20 pointer-events-none absolute -top-32 -right-24 size-80 rounded-full blur-3xl"
        />

        <div className="relative">
          <div className="flex items-center gap-2.5">
            <span className="bg-brand-500 grid size-10 place-items-center rounded-xl">
          ...
          <h1 className="mt-10 max-w-xl text-3xl leading-tight font-semibold tracking-tight text-white sm:text-4xl">
            Consent, asked properly.
          </h1>
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-slate-400">
            A working demo of Akku Consent inside a retail bank — onboarding, KYC and a privacy centre
            ...
            {HIGHLIGHTS.map(({ Icon, title, body }) => (
              <li key={title} className="flex gap-3.5">
                <span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg bg-white/[0.07]">
                  <Icon className="text-brand-300 size-4" strokeWidth={1.9} aria-hidden />
                </span>
                <span>
          ...
          <div className="mt-8">
            ...
            <div className="space-y-2">
              {DEMO_USERS.map((demo) => (
                ...
                >
                  <span className="bg-brand-50 text-brand-700 grid size-9 shrink-0 place-items-center rounded-full text-xs font-bold">
                    {demo.name
                      .split(' ')
                      .map((part) => part[0])
                      .join('')}
                  ...
                  <span className="text-brand-600 text-2xs shrink-0 font-semibold">Use</span>
                </button>
              ))}
            </div>
          </div>
